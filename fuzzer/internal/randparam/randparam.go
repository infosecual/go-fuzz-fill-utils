...
//
// The primary use case is to allow infosecual/go-fuzz-fill-utils to automatically generate fuzzing functions
...
// was targeted at use by infosecual/fzgo, which was a working prototype of an earlier "first class fuzzing in cmd/go" proposal,
...
// 		return 1
// 	default:
// 		panic(fmt.Sprintf("go-fuzz-fill-utils: kindBytes: unexpected kind %v", k))
// 	}
// }
...
// 		// TODO: handle complex?
// 		panic("go-fuzz-fill-utils: fillNumeric: complex not yet implemented")
// 	default:
// 		panic(fmt.Sprintf("go-fuzz-fill-utils: fillNumeric: unexpected kind %v for value %v of type %v", v.Kind(), v, v.Type()))
// 	}
// }
...
func (f *Fuzzer) Fill2(obj interface{}) {
	...
	if v.Kind() != reflect.Ptr {
		panic("go-fuzz-fill-utils: Fill requires pointers")
	}
...
func (f *Fuzzer) fill(v reflect.Value, depth int, opts fillOpts) {
	...
	case reflect.Interface:
		...
		if !success && opts.panicOnUnsupported {
			panic(fmt.Sprintf("go-fuzz-fill-utils: fill: unsupported interface kind %v for value %v of type %v", v.Kind(), v, v.Type()))
		}
	case reflect.Ptr:
	...
	case reflect.Uintptr, reflect.Chan, reflect.Func, reflect.UnsafePointer:
		if opts.panicOnUnsupported {
			panic(fmt.Sprintf("go-fuzz-fill-utils: fill: unsupported kind %v for value %v of type %v", v.Kind(), v, v.Type()))
		}
	case reflect.Invalid:
		panic("go-fuzz-fill-utils: fill: reflect.Invalid object")
	default:
		panic(fmt.Sprintf("go-fuzz-fill-utils: fill: unexpected kind %v for value %v of type %v", v.Kind(), v, v.Type()))
	}
}
...
func (f *Fuzzer) numericDraw(k reflect.Kind) (bits uint64) {
	...
	case reflect.Int8, reflect.Uint8:
		...
		bits = uint64(f.fzgoSrc.Byte())
	default:
		panic(fmt.Sprintf("go-fuzz-fill-utils: numericDraw: unexpected kind %v", k))
	}
	return bits
}
