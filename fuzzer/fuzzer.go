...
//
// Package fuzzer can be used completely independently from the go-fuzz-fill-utils command
// by manually constructing fuzzing functions, or the go-fuzz-fill-utils command can
...
import (
	...
	"github.com/infosecual/go-fuzz-fill-utils/fuzzer/internal/randparam"
)
...
func (fz *Fuzzer) Fill(x ...interface{}) {
	...
	for _, arg := range x {
		...
		if debugPrintPlan {
			fmt.Printf("go-fuzz-fill-utils: filled object of type \"%T\" using %d bytes. %d bytes remaining.\n",
...
func (fz *Fuzzer) Chain(steps []Step, options ...ChainOpt) {
	...
	if debugPrintPlan {
		emitPlan(pl)
		fmt.Printf("go-fuzz-fill-utils: filled Plan using %d bytes. %d bytes remaining.\n",
...
func (fz *Fuzzer) chain(steps []Step, pl plan.Plan) {
	...
	if debugPrintPlan {
		fmt.Printf("go-fuzz-fill-utils: parallelPlan byte: %v startParallelIndex: %d stopParallelIndex: %d sequential: %v\n",
...
func (fz *Fuzzer) callStep(ec execCall) []reflect.Value {
	...
	if len(ret) != ec.fv.Type().NumOut() {
		panic("go-fuzz-fill-utils: mismatch on return value count")
	}
	if len(ret) != len(ec.outputSlots) {
		panic(fmt.Sprintf("go-fuzz-fill-utils: for execCall %v, mismatch on return value count vs. execCall.outputSlots count: %+v, %+v", ec.name, ret, ec.outputSlots))
	}
		...
		for i := 0; i < ec.fv.Type().NumOut(); i++ {
			if ec.outputSlots[i].needed {
				...
				if outT != outV.Type() || outT != ec.outputSlots[i].typ {
					panic("go-fuzz-fill-utils: mismatch on return value types")
				}
...
func mustFunc(obj interface{}) reflect.Value {
	...
	if fv.Kind() != reflect.Func {
		panic(fmt.Sprintf("go-fuzz-fill-utils: Step.Func is not of type func. [kind: %v %%T: %T value: %v]", fv.Kind(), fv, fv))
	}
	return fv
}
...
// that does not have any dependency on go-fuzz-fill-utils/fuzzer or testing.F.
//
