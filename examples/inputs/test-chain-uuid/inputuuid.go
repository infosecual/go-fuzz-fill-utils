...
// NewFromBytes should be picked up as the constructor by default via 'go-fuzz-fill-utils -chain'.
func NewFromBytes(b []byte) (uuid MyUUID, err error) { return MyUUID{}, nil }

// SetNodeID is a package-level func, and by default should NOT be included via 'go-fuzz-fill-utils -chain'.
// Currently fails (is included)
func SetNodeID(id []byte) bool { return false }

// URN has a non-pointer receiver, and by default SHOULD be included via 'go-fuzz-fill-utils -chain'.
// Currently fails (is missing)
func (uuid MyUUID) URN() string { return "" }

// UnmarshalBinary has a pointer receiver, and by default SHOULD be included via 'go-fuzz-fill-utils -chain'.
...
// MyUUID2 is a second type that should also be picked up via 'go-fuzz-fill-utils -chain'.
type MyUUID2 struct{}
...
// MyUUID4 should not be picked up via 'go-fuzz-fill-utils -chain' because there is no constructor.
type MyUUID4 struct{}
