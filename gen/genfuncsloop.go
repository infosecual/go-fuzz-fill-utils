...
import (
	...
	"github.com/infosecual/go-fuzz-fill-utils/gen/internal/mod"
)
...
func emitChainWrappers(pkgPath string, pkgFuncs *pkg, wrapperPkgName string, options wrapperOptions) ([]byte, error) {
	...
	emit("\t\"github.com/infosecual/go-fuzz-fill-utils/fuzzer\"\n")
	emit(")\n\n")
...
func emitChainWrapper(emit emitFunc, functions []mod.Func, possibleConstructors []mod.Func, options wrapperOptions) error {
	...
	sort.Slice(functions, func(i, j int) bool {
		...
		//   func (github.com/infosecual/fzgo/genfuzzfuncs/examples/test-constructor-injection.A).ValMethodWithArg(i int) bool
	...
	for _, flavor := range []string{"Binary", "Text"} {
		for _, function := range functions {
			...
			if err != nil {
				fmt.Fprintln(os.Stderr, "go-fuzz-fill-utils: warning: continuing after failing to check encoding.Marshal capabilities:", err)
				continue
			}
			...
			if err != nil {
				fmt.Fprintln(os.Stderr, "go-fuzz-fill-utils: warning: continuing after failing to check encoding.Unmarshal capabilities:", err)
				continue
			}
			if implements && flavor == "Binary" {
	...
	if doBinaryRoundtrip || doTextRoundtrip {
		...
		if !ok {
			fmt.Fprintln(os.Stderr, "go-fuzz-fill-utils: warning: continuing after failing to determine type of target for roundtrip checks")
			doBinaryRoundtrip = false
			doTextRoundtrip = false
		}
		ctorResults := ctorSig.Results()
		if ctorResults.Len() < 1 {
			fmt.Fprintln(os.Stderr, "go-fuzz-fill-utils: warning: skipping roundtrip checks")
			doBinaryRoundtrip = false
			doTextRoundtrip = false
		}
...
func emitChainTarget(emit emitFunc, function mod.Func, qualifyAll bool) error {
	...
	if recv == nil {
		...
		if err != nil {
			...
			fmt.Fprintf(os.Stderr, "go-fuzz-fill-utils: warning: createWrapper: failed to determine receiver type: %v: %v\n", recv, err)
			return nil
		}
...
func emitChainStep(emit emitFunc, function mod.Func, constructor mod.Func, qualifyAll bool) error {
	...
	// Start emitting the wrapper function, inside of a go-fuzz-fill-utils/fuzzer.Step. Will be similar to:
	//   Step{
	// 	   Name: "input int",
	// 	   Func: func(a int) int {
	//	          return a
