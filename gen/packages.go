...
func findFuncs(pkgPattern, funcPattern string, env []string, flags findFuncFlag) ([]mod.Func, error) {
	...
	cfg := &packages.Config{
		...
		// With that change, 'go-fuzz-fill-utils -pkg=github.com/google/uuid' from an empty directory in
...
func goListDir(pkgPath string, env []string) (string, error) {
	...
	if err != nil {
		...
		fmt.Fprintf(os.Stderr, "go-fuzz-fill-utils: 'go list -f {{.Dir}} %v' failed for pkgPath %q\n%v\n", pkgPath, pkgPath, string(out))
...
func goList(dir string, args ...string) ([]string, error) {
	...
	if err != nil {
		// If this fails with a pkgPath as empty string, check packages.Config.Mode
		fmt.Fprintf(os.Stderr, "go-fuzz-fill-utils: 'go list' failed for args %q:\n%s\n", args, string(out))
...
func isExportedFunc(f *types.Func) bool {
	...
	if err != nil {
		...
		fmt.Fprintf(os.Stderr, "go-fuzz-fill-utils: warning: failed to determine if exported for receiver %v for func %v: %v\n",
			recv, f, err)
		return false
	}
