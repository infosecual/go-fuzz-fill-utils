...
// for the go-fuzz-fill-utils command.
//
...
// one way to test this is against the stdlib (here, this just tests that go-fuzz-fill-utils generates and the result compiles successfully):
//   for x in $(go list std | egrep -v 'internal|runtime|unsafe|vendor|image/color/palette'); do echo $x; mkdir -p $x; cd $x ; go-fuzz-fill-utils $x && gotip test -c || echo "--- FAILED $x ---"; cd - >/dev/null; done &> out.txt
// current stats:
...
var Usage = `
Usage:
	go-fuzz-fill-utils [-chain] [-parallel] [-ctor=<target-constructor-regexp>] [-unexported] [package]
	
Running go-fuzz-fill-utils without any arguments targets the package in the current directory.

go-fuzz-fill-utils outputs a set of wrapper fuzz functions for all functions matching
...
go-fuzz-fill-utils supports a package pattern as the last argument. If multiple packages match,
...
func FzgenMain() int {
	...
	switch {
	case flag.NArg() > 1:
		fmt.Println("go-fuzz-fill-utils: only one package pattern argument is allowed, and it must match only one package")
		flag.Usage()
		return 2
	case flag.NArg() == 1:
	...
	if *parallelFlag && !*chainFlag {
		fmt.Fprint(os.Stderr, "go-fuzz-fill-utils: -parallel flag requires -chain\n")
		return 2
	}
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "go-fuzz-fill-utils: %v\n", err)
		return 1
	}

	// Check if we are looking at one package vs. multiple.
	if len(pkgs) > 1 && hasPath(*outFileFlag) {
		fmt.Fprint(os.Stderr, "go-fuzz-fill-utils:-o can only specify a file name and not a path when the package pattern matches multiple packages\n")
		return 2
	}
	...
	for i := range pkgs {
		...
		msgDest, msgPrefix := os.Stderr, "go-fuzz-fill-utils:"
		if len(pkgs) > 1 {
			msgDest, msgPrefix = os.Stdout, fmt.Sprintf("go-fuzz-fill-utils: skipping %s:", pkgs[i].pkgPath)
		}
		switch {
		...
		if err != nil {
			fmt.Fprintln(os.Stderr, "go-fuzz-fill-utils: warning: continuing after failing to find abs path:", err)
			abs = outFile
		}
		adjusted, err = imports.Process(abs, out, nil)
		if err != nil {
			fmt.Fprintln(os.Stderr, "go-fuzz-fill-utils: warning: continuing after failing to automatically adjust imports:", err)
			adjusted = out
		}
		...
		fmt.Println("go-fuzz-fill-utils: created", rel)
	}

	if generatedFiles > 1 {
		fmt.Printf("go-fuzz-fill-utils: created %d files\n", generatedFiles)
	}
...
func fail(err error) {
	...
	if ok {
		...
		fmt.Fprintf(os.Stderr, "go-fuzz-fill-utils: %s:%d: error: %v\n", file, line, err)
	} else {
		fmt.Fprintf(os.Stderr, "go-fuzz-fill-utils: error: %v\n", err)
	}
	os.Exit(1)
}
...
// Edit if desired. Code generated by "go-fuzz-fill-utils %s".
